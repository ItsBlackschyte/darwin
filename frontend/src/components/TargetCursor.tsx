...
/**
 * Reticle cursor, scoped: it only appears while the mouse is inside
 * `scopeSelector` (the specimen containment box). Elsewhere the normal
 * cursor is untouched. Elements matching `targetSelector` inside the
 * scope get the lock-on corner animation.
 */
...
const getContainingBlock = (element: HTMLElement | null): HTMLElement | null => {
  ...
  while (node && node !== document.documentElement) {
    ...
    if (
      ...
      /paint|layout|strict|content/.test((style as any).contain ?? "")
    ) {
      return node;
    }
...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    gsap.set(cursor, {
      ...
      autoAlpha: 0, // hidden until the mouse enters the scope
    });

    // ---- scope: reticle exists only inside the containment box ----
    const scopeEl = document.querySelector(scopeSelector) as HTMLElement | null;
    const scopeEnter = () => gsap.to(cursor, { autoAlpha: 1, duration: 0.15 });
    const scopeLeave = () => gsap.to(cursor, { autoAlpha: 0, duration: 0.15 });
    if (scopeEl) {
      scopeEl.addEventListener("mouseenter", scopeEnter);
      scopeEl.addEventListener("mouseleave", scopeLeave);
    }
    ...
    return () => {
      ...
      if (scopeEl) {
        scopeEl.removeEventListener("mouseenter", scopeEnter);
        scopeEl.removeEventListener("mouseleave", scopeLeave);
      }
      if (activeTarget) cleanupTarget(activeTarget);
      spinTl.current?.kill();
      isActiveRef.current = false;
